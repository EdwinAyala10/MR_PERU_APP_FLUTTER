...
class TagRowCompanies extends ConsumerStatefulWidget {
  const TagRowCompanies({super.key});

  @override
  _TagRowState createState() => _TagRowState();
}

class _TagRowState extends ConsumerState<TagRowCompanies> {
  bool showAllTags = false;

  @override
  Widget build(BuildContext context) {
    final List<FilterOption> allFilters =
        ref.watch(companiesProvider).filtersSuccess;

    // Filtrar para excluir filtros de distancia (se manejan de forma independiente)
    final List<FilterOption> listFiltersSuccess =
        allFilters.where((filter) => filter.type != 'DISTANCIA').toList();
    ...
    return Container(
      ...
      child: Row(
        children: [
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return TagItem(
                        ...
                        onDelete: () {
                          ...
                          final realIndex = allFilters.indexOf(filter);
                          ref
                              .read(companiesProvider.notifier)
                              .onDeleteFilter(realIndex);
                        },
                      );
                    },
                  ),
          ...
          InkWell(
            onTap: () {
              ref.read(companiesProvider.notifier).onDeleteAllFilter();
            },
